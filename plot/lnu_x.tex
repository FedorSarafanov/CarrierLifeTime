...
\begin{document}
	\begin{tikzpicture}
	  ...
	  %          {$\pgfmathprintnumber[frac, frac denom=32,frac whole=false]{\pgfmathresult}\pi$}
	     ...
	     % \xdef\C{5e-8}
	     % \xdef\R{13000}
		% \addplot +[mark=o, line width=0.7pt, only marks, mark size=1.5pt, black] plot [error bars/.cd, y dir = both, y explicit] table 
		% [		    	    	    	
		% 	x=u, 
		% 	y=i, 
		% 	% y error=dz
		% ] 
		% {../data/current-voltage.tsv};
		...
		[		    	    	    	
			...
			%     create col/expr={
			%     abs(\thisrow{u}/100*0.5)+0.3
			%     }
			% },	
			create on use/lnu/.style={
			    create col/expr={
			    ln(\thisrow{U}*(\thisrow{x}>82.25?1:2.5))
			    }
			},	
			% create on use/yerr/.style={
			%     create col/expr={
			%     abs((\thisrow{i}>82.25?10/100*2.5:3/100*2.5))
			%     }
			% },	
			% y error=yerr,
			% x error=xerr
		...
		{../data/difflen.tsv};
	     ...
	     % \addplot [samples=100, domain=0:27, blue] {0.375*x};
	     \addplot [samples=100, domain=79:84, blue!50!black] {47.6677-0.569931*x};
	        \addlegendentry{Аппроксимация}
	     % \addlegendentry{$I=0.375 U$}
